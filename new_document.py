...
app = Flask(__name__)
# It's good practice to configure CORS more securely in production, 
...
# Using a standard, effective model. Your original model is also fine.
...
def status():
    ...
    return jsonify({"message": "The Docuguard Backend is running"})



@app.route('/api/verify/noi_index', methods=['POST'])
def verify_noi_documents():
    """
    API endpoint to verify NOI INDEX II documents.
    Expects 'originalFile' and 'suspectedFile' in a multipart/form-data request.
    """
    if 'originalFile' not in request.files or 'suspectedFile' not in request.files:
        return jsonify({"error": "Missing one or both document files. Ensure keys are 'originalFile' and 'suspectedFile'."}), 400

    original_file = request.files['originalFile']
    suspected_file = request.files['suspectedFile']

    if original_file.filename == '' or suspected_file.filename == '':
        return jsonify({"error": "One or both files are empty"}), 400

    original_mime_type = mimetypes.guess_type(original_file.filename)[0] or 'application/octet-stream'
    suspected_mime_type = mimetypes.guess_type(suspected_file.filename)[0] or 'application/octet-stream'

    original_blob = {'mime_type': original_mime_type, 'data': original_file.read()}
    suspected_blob = {'mime_type': suspected_mime_type, 'data': suspected_file.read()}
...
1. **OCR & Translation**  
   - Apply OCR with multilingual support (English + Marathi).  
   - Translate Marathi headers/labels into English.  
   - Normalize field names to a common schema.

2. **Column Matching**  
   - Identify **paired columns** (same meaning, different language).  
...
3. **Value Comparison**  
   - Compare paired values **numerically/textually**.  
...
---
    ...
    try:
        ...
        return jsonify({"error": "Failed to analyze documents with the AI model.", "details": str(e)}), 500

@app.route('/api/verify/MODT', methods=['POST'])
def verify_modt_documents():
    """
    API endpoint to verify MODT documents.
    Expects 'originalFile' and 'suspectedFile' in a multipart/form-data request.
    """
    if 'originalFile' not in request.files or 'suspectedFile' not in request.files:
        return jsonify({"error": "Missing one or both document files. Ensure keys are 'originalFile' and 'suspectedFile'."}), 400
